...
const ImageArea = ({
  ...
  const handlePanResponderReleaseResolve = (changedTouchesCount: number) => {
    // 1. Check for swipe dismissal first
    if (
      swipeToDismiss &&
    ...
    // 2. If the image is zoomed out (< 1), move to center
    if (_scale.current < INITIAL_SCALE) {
      animateToPosition({ x: 0, y: 0 });
      return;
    }
  
    // 3. If the image is zoomed in (> 1), clamp position
    if (_scale.current > INITIAL_SCALE) {
    ...
    // 4. Otherwise (scale === 1), move back to center
    animateToPosition({ x: 0, y: 0 });
  
    // And also reset background opacity to 1
      ...
      <Animated.View
        style={{
          transform: [
            ...
            {
              translateY: Animated.subtract(animatedPosition.y, height),
            },
          ],
